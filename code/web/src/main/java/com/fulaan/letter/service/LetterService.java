package com.fulaan.letter.service;
...
public class LetterService {
    ...
    /** 返回k6kt小助手用户的所用最近信件列表（即跟其他用户的最近信件）
     * @param userList
     * @param page
     * @param size
     * @return
     * @throws Exception
     */
    public List<LetterRecordEntry> getLetterRecordList(List<ObjectId> userList,int page,int size) throws Exception{
//        if(!ObjectId.isValid(userId))
//        {
//            throw new IllegalParamException("the id ["+userId+" ] is valid!!");
//        }
        List<LetterRecordEntry> letterRecordEntryList = letterRecordDao.getList(userList, page * size, size);
    ...
    /**返回k6kt用户最新信件列表的数量 （不超100000）
     * @param userList
     * @return
     * @throws Exception
     */
    public int countLatestLetterList(List<ObjectId> userList)throws Exception{
        return letterRecordDao.countList(userList);
    }
