        ...
        function fillCompetitionList(srcData) {
            ...
            for (var i = 0; i < srcData.length; i++) {
                ...
                var htmlStr = "<tr flag='" + o.competitionId + "'><td>" + (i + 1) + "</td><td>" + o.competitionName + "</td><td>" + o.competitionPostscript + "</td>" +
                    "<td>" + rangeStr + "</td><td><a class='config'>设置</a></td><td><a class='editBatch'>" + o.competitionBatches.length + "</a></td><td><span class='edit'></span>&nbsp;<span class='delete'></span></td></tr>";
...
//                    	fillCompetitionList(data.competitions);
        ...
        $(".add-pb").click(function () {
            $("#pingbi-all").append("<div detailId='new' class='pingbi-new'><span>评比名称</span><input type='text' maxlength='10' oldvalue='' name='itemDetailName'><i class='i-dell'>删除</i></div>");
        });
        ...
        $("#editItemList").delegate(".span-detail", "click", function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.itemDetails) {
                        $.each(data.itemDetails, function (i, item) {
                            html += '<div detailId="' + item.id + '" class="pingbi-new"><span>评比名称</span>';
                            html += '<input type="text" name="itemDetailName" maxlength="10" oldvalue="' + item.itemDetailName + '" value="' + item.itemDetailName + '">';
                            html += '<i class="i-dell">删除</i></div>';
                        });
                    }

                    if (html == "") {
                        html = "<div detailId='new' class='pingbi-new'><span>评比名称</span><input type='text' maxlength='10' oldvalue='' name='itemDetailName'><i class='i-dell'>删除</i></div>";
                    }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (dataBatches.length > 0) {
                        ...
                        var headHtml = "<tr><th>排名</th><th> 年级 </th><th>班级</th><th>总分 </th>";
                        ...
                        detailHtml = detailHtml + "<tr class='hz-title'><td width='72'><div class='div72'>排名</div></td><td width='67'><div class='div67'></div>年级 </td><td width='57'><div class='div57'>班级</div></td><td width='71'> <div class='div71'>总分</div> </td>";
                        for (var i = 0; i < batchNum; i++) {
                        ...
                        for (var i = 0; i < dataScores.length; i++) {
                            ...
                            detailHtml = detailHtml + "<td width='67'>" + gradeName + " </td><td width='57'>" + subData.className +
                                "</td><td width='71'>" + subData.averageScore + "</td>";
