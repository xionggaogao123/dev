...
$(function() {
    ...
    $('.retrieval-search').change(function() {
        ...
        var status = (window.location.href.indexOf('/cloudClass') > 0)?$('.sty-container .sty_span_actv').attr('sty-id'):$('.retrieval-search').val();
    	...
    	$.ajax({
            ...
            success: function(data) {
                ...
                if($('#knowledge-ul').height() > 30){
                	$('#knowledge-ul').before('<div class="knowledge-down more-knowledge">更多<i class="fa fa-angle-down"></i></div>');
                }else{
    ...
    //小学 初中 高中
    $('.sty_span','.sty-container').click(function(){
    	$(this).addClass('sty_span_actv').siblings().removeClass('sty_span_actv');
    ...
    function showdata(data) {
        ...
        if (pointList.length > 0) {
            for (var i = 0; i < pointList.length; i++) {
                var content = '';
                content += "<li data-pointId='" + pointList[i].id + "'>" + pointList[i].name + "</li>";
    ...
    /*$.fn.zTree.init($("#schoolDirUl"), {
     async: {
     enable: true,
     url: '/directory/getSchoolDirTree',
     autoParam: ['id']*/
    /*,
...
/*function changeEducation(status) {
 switch (status) {
 case "1":
 {
 $('.retrieval-grade').show();
 ...
 $('.retrieval-knowledge>ul').hide();
 $('.primary-school').show().attr('id', 'knowledge-ul');
 $('.knowledge-title').text('模块');
 break;
 }
 case "2":
 {
 $('.retrieval-grade').show();
 $('.xiaoxue').hide();
 $('.chuzhong,.middle').show();
 $('.high,.low').hide();
 $('.retrieval-knowledge').find('#knowledge-ul').attr('id', '');
 $('.retrieval-knowledge>ul').hide();
 $('.middle-school').show().attr('id', 'knowledge-ul');
 $('.knowledge-title').text('知识点');
 break;
 }
 case "3":
 {
 $('.retrieval-grade,.low').hide();
 $('.middle,.high').show();
 $('.retrieval-knowledge').find('#knowledge-ul').attr('id', '');
 $('.retrieval-knowledge>ul').hide();
 $('.high-school').show().attr('id', 'knowledge-ul');
 $('.knowledge-title').text('知识点');
 break;
 }
 }
...
function refreshCloud() {
    var $knowledgeList = $('.knowledge-ul');
    //$('.retrieval-search').val();
    var status = (window.location.href.indexOf('/cloud/cloudLesson') > 0)?$('.sty-container .sty_span_actv').attr('sty-id'):$('.retrieval-search').val();
...
//                $(".info p:first").html(data.userName + '<span style="font-size:14px;">' + tname + '</span><a href="/editphoto"><img src="/img/edit.png"/></a>'
//                		+'<label id="user-help" style="margin-left: 30px;" onclick="gotoHelpPage()">用户手册</label>');
...
function getSubjectCourseInfo(word, page, size) {
    ...
    $('#y_courses').html('');
    var subjectId = $('.subject-ul .active').attr('data-subjectId') || null;
    var schoolLevel = (window.location.href.indexOf('/cloudLesson') > 0)?$('.sty-container .sty_span_actv').attr('sty-id'):$('.retrieval-search').val();//云课程页面  其他页面
    ...
    /*
     ...
     $('#y_courses').append('<span id="nodata-msg" style="position:absolute;top:50%;left:50%;">Oh~NO,目前没有符合条件的课程</span>');
     }
     var total = data.rows.length > 0 ? data.total : 0;
     var to = Math.ceil(total / size);
     totalPages = to == 0 ? 1 : to;
     ...
     error: function(e) {
     console.log('selSubjectCourseInfo error');
     },
     complete: function() {
     $('#y_courses').removeClass('hardloadingb');
     }
...
function showSubjectCourseInfo(data) {
    ...
    if (data.rows) {
        ...
        for (var i in data.rows) {
            ...
            // html += '<div class="col-xs-3 subject-container"><img class="subject-img" src="' + cimg + '">';
