...
public class FunctionClassRoomDao extends BaseDao {
     ...
     * @author huanxiaolei@ycode.cn
     */
    public void updateClassRoomEntry(ObjectId classRoomId, int number, String classRoomName, ObjectId administratorId, String administratorName) {
        DBObject query = new BasicDBObject(Constant.ID, classRoomId);
        DBObject updateValue = new BasicDBObject(Constant.MONGO_SET, new BasicDBObject("num", number).append("cn", classRoomName).append("aid", administratorId).append("an", administratorName));
        update(MongoFacroty.getAppDB(), Constant.COLLECTION_FUNCTION_CLASSROOM, query, updateValue);
    ...
    public List<ClassRoomEntry> findClassRooms(ObjectId administratorId, ObjectId schoolId, int skip, int size) {
        ...
        if (administratorId != null) {
            query.append("aid", administratorId);
        }
        if (schoolId != null) {
