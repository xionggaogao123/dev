...
public class WebIndexController extends BaseController {
    ...
    @ApiOperation(value = "获得首页兴趣列表", httpMethod = "POST", produces = "application/json")
    @ApiResponses( value = {@ApiResponse(code = 200, message = "Successful — 请求已完成",response = String.class),
            @ApiResponse(code = 400, message = "请求中有语法问题，或不能满足请求"),
            @ApiResponse(code = 500, message = "服务器不能完成请求")})
    @RequestMapping("/getMyForumList")
    @ResponseBody
    public String getMyForumList(){

        RespObj respObj=new RespObj(Constant.FAILD_CODE);
        try {
            respObj.setCode(Constant.SUCCESS_CODE);
            List<FSectionCountDTO> fSectionDTOs = fSectionService.getFSectionList(2);
            respObj.setMessage(fSectionDTOs);
        } catch (Exception e) {
            e.printStackTrace();
            respObj.setCode(Constant.FAILD_CODE);
            respObj.setErrorMessage("获得首页兴趣列表失败!");
        }
