...
public class EasemobUserAPI {
     ...
     * @param host     IP或者域名
     * @param appKey   easemob-demo#chatdemo
     * @param body 封装了用户属性的json对象
     * @param token    admin级别token
     * @return
     */
    public static String createAllUser(String host, String appKey, List body,
                                       String token) {
        String orgName = appKey.substring(0, appKey.lastIndexOf("#"));
        String appName = appKey.substring(appKey.lastIndexOf("#") + 1);
        ...
        String reqURL = "https://" + host + "/" + rest;
        String result = HttpsUtils.sendSSLRequest(reqURL, token, HttpsUtils.List2Json(body),
                HttpsUtils.Method_POST);
        return result;
    }
     ...
     * @param port     端口
     * @param appKey   easemob-demo#chatdemo
     * @param isAdmin  org管理员token true, app管理员token false, IM用户token false
     * @param postBody POST请求体
     * @return
     */
