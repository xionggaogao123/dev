...
public class ResourceDao extends BaseDao {
     ...
     */
    public List<ResourceEntry> getResourceEntryList(int type, String name, ObjectId userId, ObjectId schoolId, Collection<ObjectId> scs, Collection<ObjectId> psbs, int skip, int limit) throws ResultTooManyException {
        BasicDBObject query = buildQuery(type, name, userId, schoolId, scs, psbs);
     ...
     * @throws ResultTooManyException
     */
    public Long countResourceEntry(int type, String name, ObjectId userId, ObjectId schoolId, Collection<ObjectId> scs, Collection<ObjectId> psbs) throws ResultTooManyException {
        BasicDBObject query = buildQuery(type, name, userId, schoolId, scs, psbs);
     ...
     * @throws ResultTooManyException
     */
    private BasicDBObject buildQuery(int type, String name, ObjectId userId,
        ...
        if (StringUtils.isNotBlank(name)) {
            ...
            if (type != -1) {
                if (type == 1 || type == 2) {
                    query.append("ty", new BasicDBObject(Constant.MONGO_IN, VIDEO_TYPES));
                }
                if (type == 3) {
                    query.append("ty", new BasicDBObject(Constant.MONGO_IN, WORD_TYPES));
                }
                if (type == 4) {
                    query.append("ty", new BasicDBObject(Constant.MONGO_IN, PPT_TYPES));
                }
                if (type == 5) {
                    query.append("ty", FileType.PDF.getType());
                }
            }
