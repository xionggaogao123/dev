...
<head>
    <meta charset="utf-8">
    <title>复兰教育社区--${pSectionName}</title>
...
<div class="container">
    <div class="post-head clearfix">
        <div class="new-theam" id="expressTheme">&nbsp;&nbsp;&nbsp;&nbsp;发表新主题</div>
        <div class="top-mk" id="data">
        </div>
    </div>
    <div class="cont-main clearfix">
        <div class="cont-left">
            <div class="post-list-sel" id="postCount">
            </div>
            <p class="p-sel">
                <select id="selectOne">
                ...
                <select id="selectTwo">
                    ...
                    <option value="3">最后发表</option>
                    <option value="4">点赞数</option>
                </select>
                <em class="em1" style="cursor: pointer" id="cream">精华</em>
                <em id="rely" style="cursor: pointer">最新</em>
            </p>
...
<script id="postListTml" type="text/template">
    ...
    <div class="post-sinp">
        ...
        <p class="p1">
            ...
            {{?}}
            {{?value.type==3}}
            <em class="box-red">回帖有赏</em>
            {{?}}
            {{?value.type==2}}
            <em class="box-red">悬赏{{=value.offeredScore}}积分</em>
            {{?}}
            {{if(value.creamText!=""){ }}<em class="jing">{{=value.creamText}}</em>{{}}}{{if(value.topText!=""){ }}
            <em class="top">{{=value.topText}}</em>{{}}}</p>
        <p class="p2">{{=value.personName}}<em></em>&nbsp;{{=value.timeText}}&nbsp;&nbsp;{{=value.classifyContent}}&nbsp;&nbsp;来自{{=value.platform}}&nbsp;&nbsp;{{?value.replyUserName!=null}}{{=value.replyUserName}}{{?}}&nbsp;&nbsp;最后回复于{{=value.replyTime}}天前
...
<script id="dataTml" type="text/template">
    {{ for(var i in it) { }}
    <p class="p1">
        <span class="span1">{{=it[i].sectionName}}</span>
        <span class="span2">今日:<em>{{=it[i].count}}</em>&nbsp;/&nbsp;</span>
        <span class="span3">主题:<em>{{=it[i].theme}}</em>&nbsp;/&nbsp;</span>
    ...
    <img src="{{=it[i].image}}" width="194" height="152">
    {{ } }}
</script>

<script id="postCountTml" type="text/template">
    {{ for(var i in it) { }}
    <span class="red-b span1 classify" value="-1">全部</span>
    <span class="classify" value="1">官方公告<em>{{=it[i].classifyOne}}</em></span>
    <span class="classify" value="2">精彩活动<em>{{=it[i].classifyTwo}}</em></span>
    <span class="classify" value="5">闲聊灌水<em>{{=it[i].classifyThree}}</em></span>
    <span class="classify" value="6">原创文章<em>{{=it[i].classifyFour}}</em></span>
    <span class="classify" value="7">其他<em>{{=it[i].classifyFive}}</em></span>
    {{ } }}
</script>
...
<script>
    seajs.use('/static/js/modules/forum/postindex.js');
</script>
</body>
</html>
