...
public class ExcellentCoursesService {
   ...
   public static final String[] arg1 = {"http://doc.k6kt.com/5b2cd0143d4df93a9389fa73.jpg",
                                        "http://doc.k6kt.com/5b2cd0143d4df93a9389fa73.jpg",
                                        "http://doc.k6kt.com/5b2cd0493d4df93a9389fde3.jpg",
                                        "http://doc.k6kt.com/5b2cd04e3d4df93a9389fdfe.jpg",
                                        "http://doc.k6kt.com/5b2cd0543d4df93a9389fe56.jpg",
                                        "http://doc.k6kt.com/5b2cd0e53d4df93a938a05b6.jpg",
                                        "http://doc.k6kt.com/5b2cd0ed3d4df93a938a05cb.jpg",
   ...
   public static final String[] arg2 = {"http://doc.k6kt.com/5b2cd0463d4df93a9389fddb.jpg",
                                        ...
                                        "http://doc.k6kt.com/5b2cd04c3d4df93a9389fdfc.jpg",
                                        "http://doc.k6kt.com/5b2cd0503d4df93a9389fe0b.jpg",
                                        "http://doc.k6kt.com/5b2cd0563d4df93a9389fe7c.jpg",
                                        "http://doc.k6kt.com/5b2cd0e73d4df93a938a05b7.jpg",
                                        "http://doc.k6kt.com/5b2cd0ef3d4df93a938a05f1.jpg",
    ...
    public String teacherNewLogin(ObjectId userId,ObjectId id) throws Exception{
        ...
        if(current>start  && current < end) {//上课中
         ...
         /*   return  "https://view.csslcloud.net/api/view/lecturer?roomid="
                    ...
                    +coursesRoomEntry.getPublisherpass();*/
            if(hourClassEntry.getRoomId()!=null&& !hourClassEntry.getRoomId().equals("")){
                return "cclive://"+coursesRoomEntry.getUserId()+"/"+hourClassEntry.getRoomId()+"/"+userName+"/"+coursesRoomEntry.getPublisherpass();
            }
...
//        }
        List<HourClassEntry> hourClassEntries = hourClassDao.getEntryList(id);
        List<ClassOrderEntry> classOrderEntries = classOrderDao.getAllEntryMap(id);
        Set<ObjectId> userIds = new HashSet<ObjectId>();
        Map<String,ClassOrderEntry> entryMap = new HashMap<String, ClassOrderEntry>();
        for(ClassOrderEntry classOrderEntry:classOrderEntries){
        ...
        for(HourClassEntry hourClassEntry:hourClassEntries){
            ...
            map2.put("label","第"+index+"课时\n是否进入|进入时间");
            map2.put("prop","a"+index);
            cols.add(map2);
        }
        for(UserEntry userEntry:userEntryList){
            ...
            for(HourClassEntry hourClassEntry:hourClassEntries){
                index2++;
                String str = hourClassEntry.getID().toString()+"&"+userEntry.getID().toString();
                ClassOrderEntry classOrderEntry = entryMap.get(str);
                if(classOrderEntry==null){
                    map3.put("a"+index2,"未购买，-");//未购买
                }else{
                    if(classOrderEntry.getType()==0){
                        String createTime = "";
                        if(classOrderEntry.getGoTime()!=0l){
                            createTime = DateTimeUtils.getLongToStrTimeTwo(classOrderEntry.getGoTime());
                        }else{
                            createTime = "未记录";
                        }
                        map3.put("a"+index2,"已进入，"+createTime);//已进入
                    }else{
                        //未进入
                        map3.put("a"+index2,"未进入，-");//已进入
                    }
                }
            }
            tableData.add(map3);
    ...
    public void exportTemplate(ObjectId id,HttpServletResponse response,
    ...
    //修改课节
    public String updateOneClass(ObjectId id,HourClassDTO dto,ObjectId userId,int type)throws Exception{
        ExcellentCoursesEntry excellentCoursesEntry = excellentCoursesDao.getEntry(id);
        if(excellentCoursesEntry==null){
            throw new Exception("课程不存在！");
        }
        ...
        if(hourClassEntry.getID()!=null){
            HourClassEntry hourClassEntry2 = hourClassDao.getEntry(hourClassEntry.getID());
            if(hourClassEntry2.getOwnId()!=null && !hourClassEntry2.getOwnId().equals(hourClassEntry.getOwnId())){//换老师
                falge = true;
            }
            //暂时用以前的直播间
        ...
        if(hourClassEntry.getIsHe()==1){//已合并的不考虑分配
            falge = false;
        }
        ...
        //分配直播间
        if(falge){//
            //long current = System.currentTimeMillis();  2018-11-26 13:59:00
            if(excellentCoursesEntry.getCreateTime()>1543211940000l){
                ExcellentCoursesService.updateNewClassRoom(excellentCoursesEntry.getID(),hourClassEntry.getID());
            }
        }
        HourClassEntry hourClassEntry1 = hourClassDao.getEntry(hourClassEntry.getID());
        return hourClassEntry1.getRoomId();
    }
//    public String createOneRoom(){
//
//    }
    ...
    public void addClassTime(HourClassEntry classEntry){
        ...
        for(UserEntry u : userEntries){
            ...
            classOrderEntry.setIsRemove(0);
            classOrderEntry.setMoney(1);
            classOrderEntry.setRole(oldClassOrderEntry.getRole());
        ...
        if(classOrderEntries1.size()>0){
            ...
            //增加每日订单   ObjectId userId,ObjectId coursesId,int type,List<ObjectId> classList,double price,String order,int source
            String or = "后台付费";
            double price2 = classEntry.getClassNewPrice();
            List<ObjectId> classList = new ArrayList<ObjectId>();
            classList.add(classEntry.getID());
            this.addMoreCoursesOrderEntry(sonIds, excellentCoursesEntry, Constant.ONE, classList, price2,or,Constant.THREE);
        }
    }

    public void addMoreCoursesOrderEntry(List<ObjectId> userIds,ExcellentCoursesEntry excellentCoursesEntry,int type,List<ObjectId> classList,double price,String order,int source){
        long orderTime = System.currentTimeMillis();
        List<ObjectId>  communityIds =this.getCommunitys3(excellentCoursesEntry.getUserId(), 1, 100);
        //List<ObjectId> communityIds = excellentCoursesEntry.getCommunityIdList();
        ObjectId schoolId = null;
        ...
        List<CoursesOrderResultEntry> coursesOrderResultEntries = new ArrayList<CoursesOrderResultEntry>();
        for(ObjectId userId : userIds){
            CoursesOrderResultEntry coursesOrderResultEntry = new CoursesOrderResultEntry(
                    orderTime,userId,schoolId,schoolName,excellentCoursesEntry.getID(),excellentCoursesEntry.getTitle(),type
                    ,classList,price,order,source);
            coursesOrderResultEntries.add(coursesOrderResultEntry);
        }
        this.addClassOrderEntryBatch(coursesOrderResultEntries);
    }
     ...
     * @param list
     */
    public void addClassOrderEntryBatch(List<CoursesOrderResultEntry> list) {
        List<DBObject> dbList = new ArrayList<DBObject>();
        for (int i = 0; list != null && i < list.size(); i++) {
            CoursesOrderResultEntry si = list.get(i);
            dbList.add(si.getBaseEntry());
        }
        //导入新纪录
        if(dbList.size()>0) {
            coursesOrderResultDao.addBatch(dbList);
        }
    }
    ...
    //分配直播间
    public static void updateNewClassRoom(ObjectId id,ObjectId oldClassId){
        ExcellentCoursesDao excellentCoursesDao = new ExcellentCoursesDao();
        CoursesRoomDao coursesRoomDao = new CoursesRoomDao();
        HourClassDao hourClassDao = new HourClassDao();
        UserClassRoomDao userClassRoomDao = new UserClassRoomDao();
        ...
        Map<ObjectId,String> map = new HashMap<ObjectId, String>();
        List<HourClassEntry> hourClassEntries = hourClassDao.getEntryList(id);
        boolean falge = true;//默认主直播间
        if(userClassRoomDao.booleanEntry(coursesRoomEntry.getRoomId())){//已用
            falge = false;
        }
        for(HourClassEntry hourClassEntry :hourClassEntries){
            String rid = hourClassEntry.getRoomId();
            if(rid!=null && !rid.equals("")){//判断主直播间是否已使用
                if(rid.equals(roomId)){
                    falge = false;
                }
            }
            if(rid!=null && !rid.equals("") && hourClassEntry.getIsHe()!=1 && !hourClassEntry.getID().equals(oldClassId)){//去除合并过的直播间和刚刚改动的课节
                map.put(hourClassEntry.getOwnId(),rid);
            }
        }
        for(HourClassEntry hourClassEntry :hourClassEntries){
            if(hourClassEntry.getRoomId()==null || hourClassEntry.getRoomId().equals("") || hourClassEntry.getID().equals(oldClassId)){
                UserClassRoomEntry userClassRoomEntry = new UserClassRoomEntry();
                ObjectId userId = hourClassEntry.getOwnId();
                userClassRoomEntry.setUserId(userId);
                userClassRoomEntry.setContactId(id);
                ...
                if(newRoomId!=null){//已存在
                    ...
                    if(falge){//主直播间未使用
                        ...
                        if(userClassRoomEntry1!=null){
                            //已有
                            hourClassEntry.setRoomId(userClassRoomEntry1.getRoomId());
                            map.put(userId,userClassRoomEntry1.getRoomId());
                        }else{
                            //无
                            falge = false;
                            hourClassEntry.setRoomId(roomId);
