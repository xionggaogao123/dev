        ...
        <dl>
          ...
          <dd>
              ...
              <span style="float: right;">
                 <button class="autoSetFZB" style="width: 140px;">自动编班非走班</button>
                 <%--<button class="autoSet" style="width: 140px;">自动选课（演示用）</button>--%>
                 <button class="importTable" style="width: 140px;">导入兴趣班选课数据</button>
              </span>
          </dd>
          <dd>
            ...
            <script type="application/template" id="subjectConfTempJs">
              ...
              <tr>
                ...
                <td>{{?value.groupId==""}}全年级{{??}}{{=value.groupName}}{{?}}</td>
                <td>{{~value.pointEntrylist:v:i}}
                  星期{{=v.x}}第{{=v.y}}节
                  {{~}}</td>
                <td>{{=value.studentsCount}}/{{=value.max}}</td>
                <td class="viewdetail" sub="{{=value.subjectName}}" cid="{{=value.zbCourseId}}">查看</td>
              </tr>
              {{~}}
            </script>
          </dd>
...
<script>
  seajs.use('interestChooselesson');
</script>
</body>
</html>
