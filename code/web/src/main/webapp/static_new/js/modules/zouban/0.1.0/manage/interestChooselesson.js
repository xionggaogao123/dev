...
define(['doT', 'common', 'jquery'],function (require, exports, module) {
    ...
    $(document).ready(function()
    {
        ...
        $("body").on("click",".viewdetail",function(){
            ...
            var subjectName=$(this).attr("sub");
            chooselesson.findInterestClass(courseId,subjectName);
        });
        //明细返回
        ...
        {
            chooselesson.autoSetInterestCourse();
        });
        //导入课表
        $(".importTable").on("click",function()
        {
            window.open("/paike/interestExcel.do","_self")
        });
        //自动编班非走班
        $(".autoSetFZB").on("click",function()
        {
            chooselesson.autoArrangeFZB();
        })
    });
    //自动编排非走班
    chooselesson.autoArrangeFZB=function()
    {
        var term = $("#term").html();
        var gradeId = $("#gradeShow").attr('gid');
        $.ajax({
            url:"/paike/fzbBianban.do",
            type:"post",
            data:{
                year:term,
                gradeId:gradeId
    ...
    //模拟自动选拓展课
    chooselesson.autoSetInterestCourse=function()
    {
        var term = $("#term").html();
        var gradeId = $("#gradeShow").attr('gid');
        if(confirm("确认模拟自动选课吗？执行该操作会覆盖原有数据。")) {
            $.ajax({
                ...
                success: function (data) {
                    if (data.result == "success") {
                        chooselesson.findInterestClassList();
                    }
                    else {
                        alert(data.reason);
                    }
    ...
    //获取拓展课学生详情
    chooselesson.findInterestClass=function(courseId,subjectName)
    {
        $.ajax({
            url:"/paike/findInterestStudents.do",
            type:"post",
            data:{
                courseId:courseId,
                gradeId:$("#gradeShow").attr('gid')
            ...
            success:function(data)
            {
                ...
                $("#stulist").empty();
                Common.render({tmpl: $('#stulistTempJs'), data: {data:data,sub:subjectName}, context: '#stulist'});
            }
        })
    };
    //获取拓展课列表
    chooselesson.findInterestClassList=function()
    {
        var term = $("#term").html();
        var gradeId = $("#gradeShow").attr('gid');
        $.ajax({
            ...
            {
                $("#subjectlist").empty();
                Common.render({tmpl: $('#subjectConfTempJs'), data: {data:data}, context: '#subjectlist'});
            }
        })
    };
    chooselesson.findInterestClassList();
