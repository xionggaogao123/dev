...
public class SchoolCommunityDao extends BaseDao {
    ...
    public List<ObjectId> getTrueCommunityIdsList(List<ObjectId> objectIds) {
        BasicDBObject query = new BasicDBObject()
                .append("isr", 0); // 未删除
    ...
    public List<ObjectId> getCommunityIdsListByCommunityIds(List<ObjectId> objectIds) {
        BasicDBObject query = new BasicDBObject()
                .append("isr", 0); // 未删除
        query.append("cid",new BasicDBObject(Constant.MONGO_IN,objectIds));
        List<DBObject> dbList =
     ...
     * @return
     */
    public List<ObjectId> getSchoolIdsList(List<ObjectId> objectIds) {
        BasicDBObject query = new BasicDBObject()
                .append("isr", 0); // 未删除
        query.append("cid",new BasicDBObject(Constant.MONGO_IN,objectIds));
        List<DBObject> dbList =
        ...
        if (dbList != null && !dbList.isEmpty()) {
            for (DBObject obj : dbList) {
                entryList.add(new SchoolCommunityEntry((BasicDBObject) obj).getSchoolId());
            }
        }
        return entryList;
    }
