...
define(['doT', 'common', 'jquery', 'initPaginator', 'rome', 'fancybox'], function (require, exports, module) {
    ...
    vote.init = function () {
        ...
        $('.sure-user').click(function() {
            ...
            $('.div-zd').append("<div style='display: inline-block;line-height: 10px;'> <em>"+$('#userIds').find("option:selected").text()+"</em> <i ui="+$('#userIds').val() +" class='fa fa-times-circle fdelete-candidate'></i> </div>");

        });
        $('.chanle-user').click(function() {
            $('.sch-na').hide();
            $('.bg').hide();
        });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = list.length-1; i >=0; i--) {
                    $('#userIds').append("<option value="+list[i].id+">"+list[i].userName+"</option>");
                }
            }
        });
    }
