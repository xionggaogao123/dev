<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<div id="sidebar-wrapper">
    <ul class="sidebar-nav">
        <li class="sidebar-brand">
            <a href="#">
                复兰后台管理
            </a>
        </li>
        <li>
            <a href="/admin/addComment.do">添加评论</a>
        </li>
        <li>
            <a href="/admin/upsetGoods.do">编辑商品</a>
        </li>
        <li>
            <a href="/admin/setActivity.do">大赛帖子</a>
        </li>
        <li>
            <a href="/admin/category.do">商品分类</a>
        </li>
        <li>
            <a href="/admin/expressTemplate.do">运费模板</a>
        </li>
        <li>
            <a href="/admin/manageOrders.do">订单管理</a>
        </li>
        <li>
            <a href="/admin/manageVouchers.do">抵用券管理</a>
        </li>
        <li>
            <a href="/admin/uploadVideo.do">SETM上传视频</a>
        </li>
        <li>
            <a href="/admin/level.do">论坛等级管理</a>
        </li>
        <li>
            <a href="/admin/optimizationPlate.do">优化板块管理</a>
        </li>
        <li>
            <a href="/admin/appBannerManager.do">AppBanner管理</a>
        </li>
        <li>
            <a href="/vv/banners.do"">Banner管理</a>
        </li>
        <li>
            <a href="/admin/sectionManage.do">版主管理</a>
        </li>
        <li>
            <a href="/admin/SystemMessage.do">消息管理</a>
        </li>
        <li>
            <a href="/admin/loginInfo.do">登录数据</a>
        </li>
        <li>
            <a href="/admin/institute.do">培训数据管理</a>
        </li>
        <li>
            <a href="/admin/batchImage.do">培训图片管理</a>
        </li>
        <li>
            <a href="/admin/manageCriticism.do">培训机构管理</a>
        </li>
        <li>
            <a href="/admin/manageItemType.do">培训分类管理</a>
        </li>
    </ul>
</div>
