...
public class GoodsDao extends BaseDao {
	 ...
	 */
	public List<GoodsEntry> getGoodsList(int sub,List<Integer> grades,int type,int state,String name,List<ObjectId> users,ObjectId owner,DBObject fields,DBObject order,int skip,int limit,List<ObjectId> userids,int ismytype) throws IllegalParamException
	{
		BasicDBObject dbo = buildQuery(sub, grades, type, state,name,users, owner,userids,ismytype);
	 ...
	 * @throws IllegalParamException
	 */
	public List<GoodsEntry> getGoodsList(int sub,List<Integer> grades,int type,int state,String name,List<ObjectId> users,ObjectId owner,DBObject fields,DBObject order,List<ObjectId> userids,int ismytype) throws IllegalParamException
	{
		BasicDBObject dbo = buildQuery(sub, grades, type, state,name,users, owner,userids,ismytype);
		List<GoodsEntry> retList =new ArrayList<GoodsEntry>();
	...
	public int countGoods(int sub,List<Integer> grades,int type,int state,String name,List<ObjectId> users,
			ObjectId owner,List<ObjectId> userids,int ismytype) throws IllegalParamException
	{
		BasicDBObject dbo = buildQuery(sub, grades, type, state,name,users, owner,userids,ismytype);
	...
	private BasicDBObject buildQuery(int sub, List<Integer> grades, int type,
			int state,String name,List<ObjectId> users, ObjectId owner,List<ObjectId> userids,int ismytype) throws IllegalParamException {
...
//		updatevalue.append(Constant.MONGO_SET,new BasicDBObject("st", GoodsState.SHELVE.getType()));
	...
	{
		BasicDBObject dbo = new BasicDBObject(Constant.ID,new BasicDBObject(Constant.MONGO_GTE,new ObjectId("576c0780f446bc0ec0ef0f4f")).append(Constant.MONGO_LTE, new ObjectId("57bb2180f446bc0ec0ef0f50"))).append("ty",1);
		List<GoodsEntry> retList =new ArrayList<GoodsEntry>();
		
		List<DBObject> list=find(MongoFacroty.getAppDB(), Constant.COLLECTION_GOODS_NAME, dbo, Constant.FIELDS);
		if(null!=list && list.size()>0)
		{
			for(DBObject dBObject:list)
			{
