    ...
    * */
    public void updateExpandClass(InterestClassEntry expandClassEntry) {
        ...
        BasicDBObject update=new BasicDBObject("cn",expandClassEntry.getClassName()).
                ...
                append("sex", expandClassEntry.getSex()).
...
//    }
    ...
    public InterestClassTranscriptEntry findTranscriptByUserIdAndClassId(ObjectId userId, ObjectId classId, int termType) {
        BasicDBObject query=new BasicDBObject("cid",classId).append("uid",userId).append("tt", termType);
    ...
    public List<InterestClassEntry> findInterestClassOfSchool(ObjectId schoolID, int termType,ObjectId gradeId, Boolean stateFlag, String typeId,int sex) {
        BasicDBObject query=new BasicDBObject("sld",schoolID).append("clt", new BasicDBObject(Constant.MONGO_GTE, 1435719600000l)).append("ir", new BasicDBObject("$ne", 1));
        if(stateFlag){
            query.append("stat", 1);
        }
        ...
        if (sex!=2) {
            ...
            sexList.add(2);
            sexList.add(sex);
            query.append("sex",new BasicDBObject(Constant.MONGO_IN,sexList));
        }
        if(typeId!=null){
            if(typeId.equals("")){
                query.append("tyid", null);
