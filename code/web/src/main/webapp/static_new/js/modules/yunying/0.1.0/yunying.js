...
define(['jquery','doT','easing','common','sharedpart','uploadify','fancybox','experienceScore'],function(require,exports,module){
     ...
     * yunying.init()
     */
    yunying.init = function(){


        yunying.uploadVideo();
        yunying.homepageFileUpload();
        yunying.getThemes();
        yunying.setNotifyNum();
        //设置初始页码
        homepageData.page = 1;
        //设置每页数据长度
        ...
        $('.seachReply').click(function () {
            homepageData.type = 1;
            yunying.getFriendReplyInfo(this,1);
        });
        // 查询数据
        yunying.initHomePageData(1);
        yunying.publishSchoolMicoSecurity();
        ...
        $(".delblog").on("click", function () {
            yunying.deleteAllMicroblog();
            yunying.initHomePageData(1);
        });
        $(".iszhu").on("click", function () {
            yunying.canceltop();
            yunying.initHomePageData(1);
        });

        $("#seachindex").click(function(event) {
            yunying.initHomePageData(1);
        });
        $("#download").click(function(event) {
            window.location.href="/yunying/exportExcel.do?startDate="+$("#bTime").val()+"&endDate="+$("#eTime").val()+"&formtype="+$('#phone').val()+"&schoolid="+$('#schoollist').val()+"&keyword="+$('#keyword').val()+"&blogtype="+$('#blogtp').val()+"&hottype="+homepageData.hottype+"&choose1="+$('#choose1').val()+"&choose2="+$('#choose2').val()+"&btype=0"+"&theme=";
        });

        $(".player-close-btn").click(function(event) {
            yunying.closeCloudView();

        });

        $(".themeitem").click(function(event) {
            homepageData.theme=$(this).attr("con");
            homepageData.hottype = 1;
            $("#tab_Main").css({
                "display": 'block'
            });
            $("#PL").css({
    ...
    yunying.initHomePageData = function(type) {
        //homepageData.seachtype = $("#order").val();
        homepageData.startDate = $("#bTime").val();
        homepageData.endDate = $("#eTime").val();
        homepageData.formtype = $('#phone').val();
        homepageData.schoolid = $('#schoollist').val();
        homepageData.keyword = $('#keyword').val();
        homepageData.blogtype = $('#blogtp').val();
        homepageData.choose1 = $('#choose1').val();
        homepageData.choose2 = $('#choose2').val();
        homepageData.btype = $('#btp').val();
        if (type==1) {
            homepageData.theme = "";
        }

        Common.getData('/yunying/selBlogInfo.do',homepageData,function(rep){
            $('.hwk1').html('');
            ...
            $("#tiaozhuan").click(function(event) {
                homepageData.page=$('#yema').val();
                yunying.initHomePageData(type);
            });
            //$(".videoshow2").click(function(event) {
            //    $(".player-close-btn").css({
            //        "display": 'block'
            //    });
            //    yunying.tryPlayYCourse($(this));
            //});
        ...
        $('.del_cls').bind("click",function(event){
            if (confirm("确定要删除该条微博吗？")) {
                ...
                yunying.deleteMicroblog(1,this);
                //查询校园安全信息数据
                yunying.initHomePageData(type);
            }
        });
         ...
         * 绑定删除按钮
         * */
        $('.shoucang_cls').bind("click",function(event){
            if (confirm("确定要收藏该条微博吗？")) {
                homepageData.id = $(this).attr('id');
                yunying.shouCangMicroblog(1,this);
                //查询校园安全信息数据
                yunying.initHomePageData(type);
            }
        });
    ...
    yunying.getFriendReplyInfo=function (dom,value) {
        homepageData.startDate2 = $("#bTime2").val();
        homepageData.endDate2 = $("#eTime2").val();
        Common.getData('/yunying/friendReplyInfo.do',homepageData,function(rep){
            if (homepageData.type==1) {
                ...
                $('.del_reply').bind('click', function () {
                    if (confirm('确认删除此条评论！')) {
                        ...
                        yunying.deleteMicroblog(2,this);
    ...
    // 分页初始化
    yunying.initPaginator=function (option) {
        var totalPage = '';
        $('.page-paginator').show();
        $('.page-index').empty();
        ...
        yunying.buildPaginator(totalPage, option.currentpage);
        option.operate(totalPage);
    }

    yunying.buildPaginator =function (totalPage, currentPage) {
        if (totalPage > 5) {
            if (currentPage < 4) {
                for (var i = 1; i <= 5; i++) {
                    if (i == currentPage) {
    ...
    // 分页初始化
    yunying.initPaginator2=function (option) {
        var totalPage = '';
        ...
        yunying.buildPaginator2(totalPage, option.currentpage);
        option.operate(totalPage);
    }

    yunying.buildPaginator2 =function (totalPage, currentPage) {
        if (totalPage > 5) {
            if (currentPage < 4) {
                for (var i = 1; i <= 5; i++) {
                    if (i == currentPage) {
     ...
     * 发布一条校园安全信息
     * */
    yunying.publishSchoolMicoSecurity = function() {
        $('.orange-btn').click(function(event){
            ...
            if (yunying.getLength(content) > 1000) {
                alert('文字最多发1000个字符！');
                return;
            }
            ...
            homepageData.schoolid=$("#schooltype").val();
            //leixing    改
            homepageData.blogtype = $('#blogtp2').val();
            homepageData.sendtype = $('#sendtype').val();
            $('#picuploadLoading').show();
            Common.getData('/yunying/publicBlog.do', homepageData,function(rep){
                if (rep.score) {
     ...
     * 上传校园安全附件信息
     * */
    yunying.homepageFileUpload = function(id) {
        /*
         * 点击附件按钮
         * */
    ...
    yunying.replyBlog = function(dom) {
        Common.getPostData('/homeschool/replyComment.do', homepageData,function(rep){
            ...
            yunying.getFriendReplyInfo(dom,1);
            if (rep.score) {
                scoreManager(rep.scoreMsg, rep.score);
            }
        });
     ...
     * 删除一条微博信息
     * */
    yunying.shouCangMicroblog = function(type,dom) {
        //删除一条微博信息
        Common.getData('/yunying/shouCangMicroblog.do', homepageData,function(){
                alert("收藏成功！");
            }
        );
    }
     ...
     * 删除一条微博信息
     * */
    yunying.deleteMicroblog = function(type,dom) {
        //删除一条微博信息
        Common.getData('/homeschool/delteMicroBlog.do', homepageData,function(){
                ...
                if (type==2) {
                    yunying.getFriendReplyInfo(dom,2);
                }
            }
        );
    }
     ...
     * 删除一条微博信息
     * */
    yunying.deleteAllMicroblog = function() {
        var nidList = "";
        $("input[name='checkitem']:checked").each(function (i) {
            nidList += $(this).parent().attr('mid') + ",";
        });
        if (nidList != "") {
            nidList = nidList.substr(0, nidList.length - 1);
        }
        homepageData.ids=nidList;
        //删除一条微博信息
        Common.getData('/yunying/delteAllMicroBlog.do', homepageData,function(rep){
                if (rep.result) {
                    alert("删除评论成功！");
                }
            }
    ...
    yunying.canceltop = function() {
        ...
        homepageData.ids=nidList;
        //取消主题帖
        Common.getData('/yunying/canceltop.do', homepageData,function(rep){
                if (rep.result) {
                    alert("取消主题帖成功！");
                }
            }
     ...
     * 赞
     */
    yunying.isBlogZan = function(dom) {
    ...
    yunying.closeCloudView = function() {
    ...
    yunying.getLength = function(content) {
