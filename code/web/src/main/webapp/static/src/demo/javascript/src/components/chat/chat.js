...
module.exports = React.createClass({
    ...
    componentDidMount: function () {
        ...
        Data.getFriendList((resp) => {
          this.setState({friendList:resp.data.message});
        });

        var me = this;
        var strangerId = Demo.getParm('userId');
        if(strangerId != undefined && strangerId != '' && strangerId != null) {
            Data.getUserInfo(strangerId,(resp) => {
              var strangers = [];
              strangers.push(resp);
              me.setState({
                strangerList:strangers
              });
            });
        }
    },
            ...
            Demo.conn.listRooms({
                success: function (rooms) {
                    ...
                    for(var i=0;i<rooms.length;i++){
                      emChatIds += rooms[i].roomId + ',';
                    }

                    Data.getEmGroups(emChatIds,(resp) => {
                      me.setState({groupList: resp.message});
                    });
                    me.setState({groups: rooms});
                },
                error: function (e) {
    ...
    render: function () {
        ...
        for (var i = 0; i < this.state.friendList.length; i++) {
            id = this.state.friendList[i].id;
            props.name = id;
            windows.push(<ChatWindow id={'wrapper' + id} key={id} {...props} chatType='singleChat'
                                     updateNode={this.updateNode}
                                     className={this.state.friendList[i].id === this.state.curNode ? '' : 'hide'}
                                     name={this.state.friendList[i].nickName}/>);
        }
        ...
        for (var i = 0; i < this.state.strangerList.length; i++) {
            id = this.state.strangerList[i].userId;
            var name = this.state.strangerList[i].nickName;

            if(name === null || name ===undefined || name === '') {
              name = this.state.strangerList[i].userName;
            }
            props.name = name;
