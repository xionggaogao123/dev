...
//	}
  ...
  public List<ObjectId> getTags() {
    if(!getBaseEntry().containsKey("tag")) return new ArrayList<ObjectId>();
    BasicDBList tagIds = (BasicDBList) getSimpleObjectValue("tag");
    List<ObjectId> tags = new ArrayList<ObjectId>();
    for (Object id : tagIds) {
      tags.add((ObjectId) id);
    }
    return tags;
  }

  public List<ObjectId> getHobbys() {
    if(!getBaseEntry().containsKey("hb")) return new ArrayList<ObjectId>();
    BasicDBList list = (BasicDBList) getSimpleObjectValue("hb");
  ...
  //获取我的社区
  public List<ObjectId> getCommunitys(){
    BasicDBList list = getDbList("comms");
    List<ObjectId> objectIds = new ArrayList<ObjectId>();
    for (Object id : list) {
      objectIds.add((ObjectId) id);
    }
  ...
  //获取我的讨论组
  public List<ObjectId> getGrids(){
    BasicDBList list = getDbList("grids");
    List<ObjectId> objectIds = new ArrayList<ObjectId>();
    for (Object id : list) {
      objectIds.add((ObjectId) id);
    }
