...
'use strict';
    (function(){
        ...
        app.controller('myCtrl', function($http,$scope) {
            $scope.eGoodsCategory = {id:'', parentId:'', level:1, sort:1, image:"", mobileImage:"", mobileCategoryImg:""};
            var level=1;
            var pId="";
            ...
            function getGoodsCategory(level,pId){
                $http.get('/mall/admin/eGoodsCategorys.do', {
                    params: {level:level,parentId:pId
                    }
            ...
            $scope.addGoodsCategory = function(level){
                ...
                $.ajax({
                    url: '/mall/admin/eGoodsCategorys.do?',
                    type: 'post',
            ...
            $scope.deleteGoodsCategory = function(id,level,pId){
                $http.delete('/mall/admin/eGoodsCategorys/'+id+'.do', {
                    params: {
                    }
                }).then(function (response) {
                    getGoodsCategory(level,pId);
            ...
            $scope.updateGoodsCategory = function(id,name,parentId,level,sort){
                var self = angular.element(window.event.target);
                var url = '/mall/admin/eGoodsCategorys/'+id+'.do';
                var param = {
                    id:id,
                    name:name,
                    parentId:parentId,
            ...
            function changeSort(selfId,selfSort,anotherId,anotherSort){
                var url = '/mall/admin/eGoodsCategorys/changeSort.do';
            ...
            $scope.uploadImg = function(id){
                uploadImg(id,'/mall/admin//eGoodsCategorys/uploadImg.do');
            }

            $scope.uploadMobileImg = function(id){
                uploadImg(id,'/mall/admin//eGoodsCategorys/uploadMoblieImg.do');
            }

            $scope.uploadMobileCategoryImg = function(id){
                uploadImg(id,'/mall/admin//eGoodsCategorys/uploadMoblieCategoryImg.do');
            }
