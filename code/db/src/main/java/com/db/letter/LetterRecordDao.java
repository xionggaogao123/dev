...
public class LetterRecordDao extends BaseDao {
	 ...
	 * @return
	 */
	public List<LetterRecordEntry> getList(List<ObjectId> userList,int skip,int limit)
	{
		List<LetterRecordEntry> retlist =new ArrayList<LetterRecordEntry>();

		BasicDBList queryList =new BasicDBList();
		DBObject query1 =new BasicDBObject("ui",new BasicDBObject(Constant.MONGO_IN,userList));
		DBObject query2 =new BasicDBObject().append("lui", new BasicDBObject(Constant.MONGO_IN,userList));
		queryList.add(query1);
		queryList.add(query2);
		BasicDBObject query =new BasicDBObject(Constant.MONGO_OR,queryList);
		BasicDBObject sortQuery = new BasicDBObject("us.id",-1).append("uis.id", -1);
	...
	public int countList(List<ObjectId> userList){
