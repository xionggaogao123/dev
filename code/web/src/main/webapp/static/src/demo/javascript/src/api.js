...
var OtherLoginSheet = require('./components/fulaan/otherlogin');

var Data = require('./components/fulaan/Data');
...
module.exports = {
  ...
  appendMsg: function (msg, type) {
    ...
    if(!targetId && msg.type === 'group') { //陌生群组
      return;
    }

    if(!targetId && msg.type === 'chatroom') {// 陌生群组
      return;
    }


    if (!targetNode && msg.type === 'chat') { //收到陌生人的消息
      return;
      var isExist = false;
      for(var i=0;i<Demo.strangerIds.length;i++) {
        if(Demo.strangerIds[i] == targetId) {
          isExist = true;
        }
      }

      if(isExist) {
        Demo.strangerIds.push(targetId);
      }

      if(!Demo.strangerMsgs.hasOwnProperty(targetId)) {
        Demo.strangerMsgs[targetId] = [];
        Demo.strangerMsgs[targetId].push({msg: msg,type:type});
      } else {
        Demo.strangerMsgs[targetId].push({msg: msg,type:type});
      }

      var userIds = '';
      if(Demo.strangerIds != undefined) {
        var strangers = [];
        for(var i=0;i < Demo.strangerIds.length;i++) {
          userIds += Demo.strangerIds[i] + ',';
        }
        Data.getUserInfos(userIds,(resp) => {

          me.render(me.node, 'stranger');
        });
      }
      return;
    }
  ...
  encode: function (str) {
    ...
    NotifyError: function (msg) {
      if(lodash.endsWith(msg,'7') || lodash.endsWith(msg,'8')){
        OtherLoginSheet.show();
      }
    },
