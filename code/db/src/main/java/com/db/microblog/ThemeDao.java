...
public class ThemeDao extends BaseDao {
    ...
    public List<ThemeEntry> getThemeList() {
        ...
        list=find(MongoFacroty.getAppDB(), Constant.COLLECTION_BLOG_THEME_NAME, query, null,new BasicDBObject(Constant.ID,1));
     ...
     * @return
     */
    public List<ThemeEntry> getThemeList2() {
        List<ThemeEntry> themeEntryList = new ArrayList<ThemeEntry>();
//        BasicDBObject query =new BasicDBObject("deflg",DeleteState.NORMAL.getState());
        BasicDBObject query =new BasicDBObject();
        List<DBObject> list =new ArrayList<DBObject>();
        list=find(MongoFacroty.getAppDB(), Constant.COLLECTION_BLOG_THEME_NAME, query, null,new BasicDBObject("deflg",1));
        for(DBObject dbo:list)
        {
            themeEntryList.add(new ThemeEntry((BasicDBObject)dbo));
        }
