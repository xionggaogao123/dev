...
define(function (require, exports, module) {
    ...
    (function () {
        ...
        $('.tc-nav span').click(function () {
            $(this).addClass('cur-tc').siblings('.tc-nav span').removeClass('cur-tc');
        });
        $('.tc-nav .sp1').click(function () {
            $('.register-main').show();
            $('.login-main').hide();
        });
        $('.tc-nav .sp2').click(function () {
            $('.register-main').hide();
            $('.login-main').show();
        });
    ...
    function checkUserName() {
        ...
        var obj = userName.match(/[^\x00-\xff]/g);
        var txtLength;
        if (obj != null) {
            txtLength = obj.length + userName.length;
        } else {
            txtLength = userName.length;
        }
        if (txtLength < 6) {
            return wrong(ele, 5);
        }
    ...
    function thirdLogIn() {
        ...
        requestData.name = $.trim($('#account').val());
        requestData.pwd = $.trim($('#password').val());
        requestData.cacheThirdId = $.trim($('#cacheThirdId').val());
        requestData.verifyCode = "";
        Common.getPostData('/user/third/login.do', requestData, function (resp) {
            if ('200' == resp.code) {
                window.location = '/mall/entrance.do';
            } else {
                if (resp.message == '密码错误') {
                    resp.message = "passwordError";
                }
    ...
    function createUser() {
        var requestData = {};
        requestData.cacheThirdId = $.trim($('#cacheThirdId').val());
        Common.getPostData('/user/third/create.do', requestData, function (resp) {
            if ('200' == resp.code) {
                window.location = '/mall/entrance.do';
            } else {
                if (resp.message == '密码错误') {
