...
public class WebExcellentCoursesController extends BaseController {
    ...
    public String updateOneClass(@ApiParam(name = "dto", required = true, value = "dto") HourClassDTO dto){
        ...
        try {
            respObj.setCode(Constant.SUCCESS_CODE);
            String roomId = excellentCoursesService.updateOneClass(new ObjectId(dto.getParentId()),dto ,getUserId(),dto.getStatus());
    ...
    @ApiResponses( value = {@ApiResponse(code = 200, message = "Successful — 请求已完成",response = String.class),
            @ApiResponse(code = 400, message = "请求中有语法问题，或不能满足请求"),
            @ApiResponse(code = 500, message = "服务器不能完成请求")})
    @RequestMapping("/newBooleanBackList")
    @ResponseBody
    public RespObj newBooleanBackList(@ApiParam(name = "id", required = true, value = "课节id") @RequestParam("id") String id,
                                      @ApiParam(name = "userId", required = true, value = "用户id") @RequestParam("userId") String userId){
        RespObj respObj=new RespObj(Constant.FAILD_CODE);
        try {
            respObj.setCode(Constant.SUCCESS_CODE);
            Map<String,Object> map = excellentCoursesService.booleanBackList(new ObjectId(id), new ObjectId(userId));
            respObj.setMessage(map);
        } catch (Exception e) {
            e.printStackTrace();
            respObj.setCode(Constant.FAILD_CODE);
