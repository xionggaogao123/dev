...
public class MicroBlogDao extends BaseDao {
	...
	public List<MicroBlogEntry> getMicroBlogEntryList(ObjectId ui,DeleteState state,Platform form,ObjectId atUserId,int blogtype,int hottype, DBObject fields,String schoolid,int skip,int limit,int seachtype,List<ObjectId> classAry,String theme,int seachType) throws ResultTooManyException
	{
		...
		if(!StringUtils.isEmpty(schoolid)) {
			...
			values2.add(new BasicDBObject("si",new ObjectId(schoolid)));
			values2.add(new BasicDBObject("mtp",1));
			query.put(Constant.MONGO_OR, values2);
		}
	...
	public List<MicroBlogEntry> getBlogEntryList(ObjectId ui,DeleteState state,Platform form,ObjectId atUserId,int blogtype,int hottype, DBObject fields,String schoolid,int skip,int limit,String theme,int seachType,Long startDate,Long endDate,int formtype,String keyword,List<ObjectId> userids,String choose1,String choose2,int btype) throws ResultTooManyException
	{
		...
		} else if (Integer.valueOf(choose1)==3) {
			query.append("ili",new BasicDBObject("$size",0));
			query.append("vids",new BasicDBObject("$size",0));
		}
	 ...
	 * @return
	 */
	public int getBlogCount(ObjectId userid, int hottype, int blogtype,String schoolid,String theme,int seachType,Long startDate,Long endDate,int formtype,String keyword,List<ObjectId> userids,String choose1,String choose2,int btype) {
		BasicDBObject query =new BasicDBObject("st", DeleteState.NORMAL.getState());
		if(null!=userid && hottype==3)
		{
			query.append("ui", userid);
...
//		DBObject query = new BasicDBObject("st", DeleteState.NORMAL.getState()).append("it",Constant.ONE);
	...
	public int getFriendReplyCount(String blogid, int type, ObjectId id, long startTime, long endTime) {
		int count = 0;
		if (type == 1) {
			BasicDBObject query =new BasicDBObject("st",DeleteState.NORMAL.getState());
			query.append("at.id", id);
			BasicDBList dblist =new BasicDBList();
			if(startTime>0){
				dblist.add(new BasicDBObject("pbt", new BasicDBObject(Constant.MONGO_GTE, startTime)));
			}
			if(endTime>0) {
				dblist.add(new BasicDBObject("pbt", new BasicDBObject(Constant.MONGO_LTE, endTime)));
			}
			if(dblist.size()>0){
				query.append(Constant.MONGO_AND, dblist);
			}
	...
	public List<MicroBlogEntry> getFriendReplyInfo(String blogid, int type, ObjectId id,long startTime,long endTime, int page, int pageSize) {
		List<MicroBlogEntry> retList = new ArrayList<MicroBlogEntry>();
		List<DBObject> list =new ArrayList<DBObject>();
		BasicDBObject query =new BasicDBObject("st",DeleteState.NORMAL.getState());
		BasicDBList dblist =new BasicDBList();
	 ...
	 * @param id
	 */
	public void shouCangMicroblog(ObjectId id) {
		DBObject query =new BasicDBObject(Constant.ID,id);
		DBObject value =new BasicDBObject("sc", 1);
		BasicDBObject updateValue =new  BasicDBObject(Constant.MONGO_SET,value);
		update(MongoFacroty.getAppDB(), Constant.COLLECTION_BLOG_NAME, query, updateValue);
	}
}
