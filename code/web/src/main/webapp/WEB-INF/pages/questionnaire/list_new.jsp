...
<div id="content" class="clearfix">
  ...
  <div class="col-right">
    <div class="quest-top">
      ...
      <a href="/questionnaire#/new">新建问卷</a>
...
<script id="list_tmpl" type="text/template">
  ...
  <li>
    ...
    <span class="span4 users"><a href="/questionnaire/users.do?id={{=it[i].id}}&state={{=it[i].state & !it[i].isPlatform}}" target="_blank">{{=it[i].process}}</a></span>
    {{? it[i].state == 1}}
    <span class="span5"><a href="/questionnaire#/static/{{=it[i].id}}" target="_blank"><em>查看统计</em></a><em id="{{=it[i].id}}" class="remove">删除</em>
      {{? it[i].isPublic == 0}}
        <em qid="{{=it[i].id}}" class="public">公开</em>
        {{??}}
        <em qid="{{=it[i].id}}" class="quitpublic">取消公开</em>
        {{?}}
    </span>
    {{?? it[i].state == 2}}
      {{? it[i].isPublic == 0}}
      ...
      {{??}}
      <span class="span5"><a href="/questionnaire#/static/{{=it[i].id}}" target="_blank"><em>查看统计</em></a></span>
      {{?}}
    {{?? it[i].state == 3}}
      {{? it[i].isPublic == 0}}
      <span class="span5"><em qid="{{=it[i].id}}" class="review">查看</em></span>
      ...
      {{?}}
    {{?? it[i].state == 4}}
    <span class="span5"><a href="/questionnaire#/answer/{{=it[i].id}}"><em>填写问卷</em></a>
      {{? it[i].isPublic == 1}}<a href="/questionnaire#/static/{{=it[i].id}}" target="_blank"><em>查看统计</em></a>
      {{?}}
    </span>
    {{?}}
...
<script>
  seajs.use('/static_new/js/modules/questionnaire/0.1.0/list_new');
</script>
</body>
</html>
