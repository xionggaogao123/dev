...
<script type="text/javascript">
    ...
    $(function(){
     ...
     $(function(){
        ...
        var hiddenUserRole =jQuery("#hiddenUserRole").val();
        if(hiddenUserRole=="2")
        {
        	userChoose('dep_div');
        }	
    ...
    {
    	 $.ajax({
             url: '/notice/classes.do?type='+type+"&ist="+ist,
             type: 'get',
             ...
             success: function (res) {
               {
            	   ...
            	   {
                      if(type==1) //学生
                    	  $contain=jQuery("#student_user_group");
                      if(type==2) //家长
    ...
    {
       //	 url: '/notice/users.do?type=4&ist=5',
    	 $.ajax({
             url: '/notice/subject/users.do?type=4&ist=5',
             type: 'get',
             contentType: 'application/json',
             success: function (res) {
               {
            	  $contain=jQuery("#real_dep_user_group"); 
            	  $contain.empty();
            	  for(var i=0;i<res.length;i++)
            	  {
                	 var html =createUserGroup(res[i],5);
                	 $contain.append(html);
            	  } 
               }
             }
    ...
    function createUserGroup(obj,ty)
    {
    	try
    	{
	    	 ...
	    	 for(var j=0;j<users.length;j++)
	    	 {
	    		 ...
	    		 html+=users[j].value;
	    		 if(ty!=1 && ty!=2 && ty!=3 && ty!=5)
	    		 {
	    			 html+='<span class="contacts-aa">(班主任，任课老师，学生及家长)</span>';
	    		 }	 
    ...
    function userChoose(ug)
    {
    	...
    	var gs=["xiaoyuan_div","dep_div","teacher_div","student_div","parent_div","teacher2_div","real_dep_div"];
    ...
    function submit()
    {
    	...
    	if($voicePath)
    	{
    		var vvalue="";
        	$('.voice').each(function(){
        		vvalue+=jQuery(this).attr('url')+",";
        	});
    	    jQuery("#voiceFile").val(vvalue)
    	}
...
<body>
    ...
    <input type="hidden" id="hiddenUserRole" value="${sessionValue.userRole}"/>
	...
	<div class="inform-all">
		...
		<div class="informm-main">
			...
			<div class="inform-main">
				...
				<form id="noticeForm" action="/notice/add.do" method="POST">
		        ...
		        var ue = UE.getEditor('container');
				 ...
				 -->
					...
					<div class="inform-bottom">
						...
						<div class="infrom-right" style="overflow: visible;">
					...
					<div class="valid-time">
						<span class="valid-time-I">*有效时间</span> <span>有效时间是同步到日历中显示的时间，有效时间过后通知中自动显示为失效时间</span>
					</div>
			...
			<div class="contacts-main" style="display: none;">
				...
				<div class="contacts-middle">
					<ul>
						...
						<li id="real_dep_div_li"  onclick="userChoose('real_dep_div')">学科</li>
				...
				<div id="real_dep_div" style="display: none">
					<div class="contacts-bottom">
						<div id="real_dep_user_group" class="contacts-bottom-text"></div>
					</div>
				</div>
