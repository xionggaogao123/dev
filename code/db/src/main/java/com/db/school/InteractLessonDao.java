...
public class InteractLessonDao extends BaseDao {
	...
	public Map<ObjectId, List<InteractLessonEntry>> getInteractLessonMapByTeacherIds(List<ObjectId> userIds,BasicDBObject fields) {
		Map<ObjectId, List<InteractLessonEntry>> map=new HashMap<ObjectId, List<InteractLessonEntry>>();
		BasicDBObject dbo = new BasicDBObject();
		dbo.append("st", DeleteState.NORMAL.getState());
		if(userIds.size()>0) {
			dbo.append("ui", new BasicDBObject(Constant.MONGO_IN, userIds));
		}
		List<DBObject> list = find(MongoFacroty.getAppDB(), Constant.COLLECTION_INTERACTLESSON, dbo, fields,Constant.MONGO_SORTBY_DESC);
		if (null != list && !list.isEmpty()) {
			InteractLessonEntry e = null;
			List<InteractLessonEntry> retList =null;
			for (DBObject dbo1 : list) {
				e = new InteractLessonEntry((BasicDBObject) dbo1);
				retList=map.get(e.getUserId());
				if(retList==null){
					retList = new ArrayList<InteractLessonEntry>();
				}
				retList.add(e);
				map.put(e.getUserId(),retList);
			}
		}
		return map;
	}
