 ...
 */
/* global Config */
define('lessonWareMge',['common','pagination'],function(require,exports,module){
    /**
     *初始化参数
     */
    var lessonWareMge = {},
     ...
     * lessonWareMge.init()
     */
    lessonWareMge.init = function(){



		lessonWareMge.queryAndeFillSelection("versionTermTypeSelection","ALL");

		//给所有资源字典类型选择下拉菜单增加事件
		$('body').on('change','.versionSelection select',function(){
			lessonWareMge.queryAndeFillSelection($(this).attr("subId"),$(this).val());
		});

		//初始查询
		lessonWareMge.queryList(1);
	...
	lessonWareMge.queryAndeFillSelection=function(selectionId,parentObjId){
		if(selectionId == "0"){
			return;
		}
		...
		if(parentObjId == "ALL" && selectionId != "versionTermTypeSelection"){
			if(selectionId != "0"){
				lessonWareMge.resetLittleBrothers(selectionId);
			}
			return;
		}
		...
		$.ajax({
			...
			dataType: "json",
			success: function(data){
				lessonWareMge.fillSelection(selectionId,data.datas);
				if($selection.attr("subId") != "0"){
					lessonWareMge.resetLittleBrothers($selection.attr("subId"));
				}
			}
		});
	}
	...
	//填充下拉菜单的方法
	lessonWareMge.fillSelection = function(selectionId,srcDatas){
	...
	//置空弟弟们
	lessonWareMge.resetLittleBrothers = function(targetId){
		if(targetId == "0"){
			return;
		}else{
			var $targetSelection = $("#" + targetId);
			$targetSelection.html(allOptionHtml);
			lessonWareMge.resetLittleBrothers($targetSelection.attr("subId"));
		}
	}
	...
	lessonWareMge.queryList = function(pageNo){
		...
		Common.getPostData('/cloudres/queryCloudList.do', searchData,function(data){
			...
			for(var i=0;i<list.length;i++){
				...
				var htmlStr = '  <tr id="' + course.id + '">'+
					'                    <td>'+
					'                        <img src="' + course.imageUrl + '" href="' + course.type + '" class="img-view-dialog" style="cursor:pointer"/>'+
					'                        <em>' + course.type + '</em>'+
					'                    </td>'+
			...
			$(".img-view-dialog").click(function(){
				var id = $(this).closest("tr").attr("id");
				$("#downId").val(id);
				$(".new-page-popup").show();
			});
			...
			$(".del-btn").click(function(){
				...
				Common.getPostData('/cloudres/delLessonWare.do', delData,function(data){
					alert("操作完成!");
					lessonWareMge.queryList(searchData.pageNo);
				});
			});
		});
	}

    module.exports = lessonWareMge;
})