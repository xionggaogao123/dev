    ...
    <div class="col-right">
        <div class="tab-col">
            ...
            <div class="tab-head clearfix">
                <ul>
                    <li class="cur"><a href="javascript:;">编辑教育局</a></li>
                </ul>
            </div>

            <div class="tab-main txt-info">
                <form id="form">
                    <input type="hidden" id="id" name="id" value="${eduDto.id}"/>
                    <input type="hidden" id="userIdsStr" name="userIdsStr" value=""/>
                    <input type="hidden" id="schoolIdsStr" name="schoolIdsStr" value=""/>
                    <input type="hidden" id="educationLogo" name="educationLogo" value="${eduDto.educationLogo}"/>
                    <input type="hidden" id="oldEduLogo" name="oldEduLogo" value="${eduDto.educationLogo}"/>
                    <table width="100%">
                        <tr>
                            <th width="15%">
                                教育局名称：
                            ...
                            <td width="35%">
                                <input type="text" id="eduName" name="eduName" value="${eduDto.educationName}"/>
                            </td>
                            <th width="15%">
                                省、直辖市：
                            </th>
                            <td width="35%">
                                <select id="province" name="province">
                                    ...
                                    <c:forEach var="item" items="${provinces}">
                                        <option value="${item.id}" <c:if test="${item.id == eduDto.province}">selected="selected"</c:if>>${item.name}</option>
                                    </c:forEach>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td width="35%">
                                <select id="city" name="city">
                                    ...
                                    <c:forEach var="item" items="${citys}">
                                        <option value="${item.id}" <c:if test="${item.id == eduDto.city}">selected="selected"</c:if>>${item.name}</option>
                                    </c:forEach>
                                </select>
                            </td>
                            <th width="15%">
                            ...
                            <td width="35%">
                                <span id="logoImg"><img src="${eduDto.educationLogo}" style="width: 170px;height:35px;"></span>
                                <div id="eduLogoPic">
                        ...
                        <tr align="center">
                            <td colspan="4">
                                ...
                                <button type="button" class="cancel" onClick="javascript:history.go(-1);">取消</button>
                            </td>
                        </tr>
                    </table>
                </form>
...
<script>
    seajs.use('editEducation',function(editEducation){
        editEducation.init();
    });
</script>
</body>
