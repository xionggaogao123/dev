...
public class GroupExamUserRecordDao extends BaseDao{
    ...
    public void updateGroupExamUserRecord(ObjectId groupExamDetailId,String subjectIds){
        BasicDBObject query=new BasicDBObject()
            .append("eid",groupExamDetailId);
...
//            statuses.add(Constant.ZERO);
    ...
    public void updateGroupExamDetailGESC(ObjectId groupExamDetailId, ObjectId groupId,ObjectId examType,ObjectId subjectId,ObjectId communityId) {
        BasicDBObject query=new BasicDBObject()
            .append("eid",groupExamDetailId);
        BasicDBObject updateValue=new BasicDBObject(Constant.MONGO_SET,new BasicDBObject("gid",groupId).append("etp", examType).append("sid", subjectId).append("cmId", communityId));
        update(MongoFacroty.getAppDB(), Constant.COLLECTION_REPORT_CARD_EXAM_USER_RECORD,query,updateValue);
    }
     ...
     * @return
     */
    public List<GroupExamUserRecordEntry> getEntriesByStatus(int page,int pageSize){
        List<GroupExamUserRecordEntry> entries = new ArrayList<GroupExamUserRecordEntry>();
        BasicDBObject query = new BasicDBObject();
        List<DBObject> dbObjectList=find(MongoFacroty.getAppDB(),Constant.COLLECTION_REPORT_CARD_EXAM_USER_RECORD,query,
                Constant.FIELDS,Constant.MONGO_SORTBY_DESC,(page-1)*pageSize,pageSize);
        if(null!=dbObjectList&&!dbObjectList.isEmpty()){
            for(DBObject dbObject:dbObjectList){
                entries.add(new GroupExamUserRecordEntry(dbObject));
    ...
    public List<GroupExamUserRecordEntry> getGroupExamUserRecordsByIds(List<ObjectId> ids){
        List<GroupExamUserRecordEntry> entries=new ArrayList<GroupExamUserRecordEntry>();
        BasicDBObject query=new BasicDBObject()
                .append(Constant.ID,new BasicDBObject(Constant.MONGO_IN,ids));
        List<DBObject> dbObjectList=find(MongoFacroty.getAppDB(), Constant.COLLECTION_REPORT_CARD_EXAM_USER_RECORD,
                query,Constant.FIELDS);
        if(null!=dbObjectList&&!dbObjectList.isEmpty()){
            for(DBObject dbObject:dbObjectList){
                entries.add(new GroupExamUserRecordEntry(dbObject));
