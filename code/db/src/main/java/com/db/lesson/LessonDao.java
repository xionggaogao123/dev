...
public class LessonDao extends BaseDao {
	 ...
	 * @return
	 */
	public Map<ObjectId, LessonEntry> getLessonEntryMap(Collection<ObjectId> dirs,long startTime,long endTime,DBObject fields)
	{
		BasicDBObject query =new BasicDBObject("di",new BasicDBObject(Constant.MONGO_IN,dirs)).append("ir", Constant.ZERO);
		BasicDBList dblist =new BasicDBList();
		if(startTime>0){
			dblist.add(new BasicDBObject("lut", new BasicDBObject(Constant.MONGO_GTE, startTime)));
		}
		if(endTime>0) {
			dblist.add(new BasicDBObject("lut", new BasicDBObject(Constant.MONGO_LTE, endTime)));
		}
		if(dblist.size()>0){
...
//        BasicDBObject query=new BasicDBObject("vis",new BasicDBObject(Constant.MONGO_IN,set)).append("ir", Constant.ZERO);
	...
	public void updateWare(ObjectId lessonId, ObjectId wareId) {
		BasicDBObject query =new BasicDBObject(Constant.ID,lessonId);
		query.append("dcl.id",wareId);
		BasicDBObject updateValue=new BasicDBObject(Constant.MONGO_SET,new BasicDBObject("dcl.$.push",Constant.ONE));
		update(MongoFacroty.getAppDB(), Constant.COLLECTION_LESSON_NAME,  query, updateValue);
	}
}
