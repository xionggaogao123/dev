...
//        });
        ...
        $('.add-answer').on('click',function(){
             $('.answer-col').append(edittiku.html);
         ...
         function addjian(type,_this){
          ...
          function fillSelectionInit(){
        	  $("#selected").val(questionTopic);
        	  $("#answer_form").show();
        	  if("判断题"== questionTopic){
     			 $("#check_id").show().siblings('.answer-check').hide();
     			edittiku.html = checkAnswerHtml;
     			 var answerArr = rightAnswer.split(',');
         	 ...
         	 if("选择题"== questionTopic){
         		 $("#choose_id").show().siblings('.answer-check').hide();
         		edittiku.html = chooseAnswerHtml;
         		 var answerArr = rightAnswer.split('_');
         		 for(var i=0;i<answerArr.length;i++){
         			 if(i>0){
         				$('.answer-col').append(edittiku.html);
         			 }
         			 var subAnswerArr = answerArr[i].split(",");
         			 var answerNum = parseInt(subAnswerArr[(subAnswerArr.length-1)]);
         			 while($($('.choose-id')[i]).find("input").length != answerNum){
         				addjian(1,$($($('.choose-id')[i]).find("span").get(0)));
         			 }
         			 for(var j = 0;j < subAnswerArr.length - 1;j++){
...
//         			 $($('.choose-id input')[i]).val(answerArr[i]);
         	 ...
         	 if("主观题"== questionTopic){
         		 $("#zhug_id").show().siblings('.answer-check').hide();
         		edittiku.html = '';
         		 $(".ask-txt").off("change");
//         		UE.getEditor('myEditor3').setContent(rightAnswer, true);
         	 };
