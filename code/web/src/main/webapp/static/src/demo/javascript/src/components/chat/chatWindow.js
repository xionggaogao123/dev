...
var GroupMemberSheet = require('../fulaan/groupMember');
...
module.exports = React.createClass({
    ...
    listMember: function () {
        if (!this.state.memberShowStatus) {
            var me = this;
            Data.getGroupMembers(me.props.roomId,(resp) => {
                me.refreshMemberList(resp.message.result);
            });
    ...
    refreshMemberList: function (members) {
        this.setState({memberList: members, memberShowStatus: true});
    },

    send: function (msg) {
        msg.ext = {
            nickName:Demo.mine.nickName,
            userId:Demo.mine.id,
            avatar:Demo.mine.imgUrl
        };
    ...
    render: function () {
        ...
        for (var i = 0, l = this.state.members.length; i < l; i++) {
            ...
            return (
                ...
                show={this.state.memberShowStatus}
                onClose={this.closeGroupMember}
                emChatId={this.props.roomId}
                />
