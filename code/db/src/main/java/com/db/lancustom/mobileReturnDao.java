...
public class mobileReturnDao extends BaseDao {
     ...
     * @return
     */
    public List<PhoneCostEntry> getAllPhoneModel() {
        List<PhoneCostEntry> phoneCostEntries = new ArrayList<PhoneCostEntry>();
        BasicDBObject query=new BasicDBObject();
        query.append("isr",Constant.ZERO);
        List<DBObject> objects = find(MongoFacroty.getAppDB(),Constant.COLLECTION_PHONE_REPAIR_COST_DICTIONARY,query);
        if (objects.size() > 0){
            for (DBObject dbObject: objects){
                phoneCostEntries.add(new PhoneCostEntry(dbObject));
            }
        }
        return phoneCostEntries;
    }
    ...
    public String getCostPicUrlByPhoneModel(String phoneModel) {
        ...
        BasicDBObject query=new BasicDBObject();
        query.append("isr",Constant.ZERO);
        query.append("model",phoneModel);
        DBObject object = findOne(MongoFacroty.getAppDB(),Constant.COLLECTION_PHONE_REPAIR_COST_DICTIONARY,query);
        if (object != null){
            result = new PhoneCostEntry(object).getCostUrl();
        }
        return result;
    }
     ...
     * @return
     */
    public List<PhoneCostEntry> getPhoneRepairCostList() {
        List<PhoneCostEntry> phoneCostEntries = new ArrayList<PhoneCostEntry>();
        BasicDBObject query=new BasicDBObject();
        query.append("isr",Constant.ZERO);
        List<DBObject> objects = find(MongoFacroty.getAppDB(),Constant.COLLECTION_PHONE_REPAIR_COST_DICTIONARY,query);
