...
//                }
    ...
    public Map<String,Object> getEightHotIndexList(ObjectId userId,int page,int pageSize){
        Map<String,Object> map = new HashMap<String, Object>();
        //1.通知逻辑
        List<ObjectId>  dlist =communityService.getCommunitys3(userId, 1, 100);
        //2.老师和家长页面区分（）
        ...
        userIds.add(userId);
        dlist.add(userId);
        List<ObjectId> cntactIdList = indexPageDao.getEightPageList(dlist, userId, page, pageSize, Constant.EIGHT,userIds,role);
        int count = indexPageDao.countEightPageList(dlist, userId, Constant.EIGHT,userIds,role);
        List<IndexContentDTO> list2 = new ArrayList<IndexContentDTO>();
        List<IndexContentEntry> entryList = indexContentDao.getPageList(cntactIdList);
        List<ObjectId> oids = new ArrayList<ObjectId>();
        Map<ObjectId,IndexContentEntry> comMap = new HashMap<ObjectId, IndexContentEntry>();
