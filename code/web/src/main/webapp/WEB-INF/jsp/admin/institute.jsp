...
<layout:override name="content">
    ...
    <div id="r-result-s">对区域对应的数据的处理:<input type="text" id="parentId" size="20" placeholder="请输入区域Id,不常用"
                                            style="width:150px;"/></div>
    <button id="replaceRegion">数据处理</button>

    <div id="r-result-ss">处理残留未处理水印图片的数据:<input placeholder="请输入Id(用@隔开)" type="text" id="instituteIds"
                                                style="width:350px;"/></div>
    <button id="batchImages">处理残留图片</button>
    <br/>
    <div id="r-result-sss">
        一些数据用默认图片覆盖:<input placeholder="请输入Id(用@隔开)或者名字(用$隔开),注意:只传一个时传Id" type="text" id="idOrNames"
                           style="width:380px;"/>
    ...
    <br/>
    <div id="r-result-lll">
        开始数据Id:<input placeholder="请输入开始Id(startDataId)" type="text" id="startDataId" style="width:200px;"/>
        结束数据Id:<input placeholder="请输入结束Id(endDataId)" type="text" id="endDataId" style="width:200px;"/>
    </div>
    <button id="remainImage">残留图片批量处理</button>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('#remainImage').click(function () {
                ...
                var endDataId = $('#endDataId').val();
                $.ajax({
                    type: "GET",
                    data: {startId: startDataId, endId: endDataId},
                    url: '/train/remainImage.do',
                    async: false,
                    dataType: "json",
                    contentType: "application/x-www-form-urlencoded; charset=UTF-8",
                    success: function (resp) {
