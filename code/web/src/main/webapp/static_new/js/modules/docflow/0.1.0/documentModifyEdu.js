    ...
    function getOnlyDepartmentList() {
        ...
        for (var i = 0; i < userlist.length; i++) {
            //for (var j = 0; j < userlist[i].list.length; j++) {
                if (checkIfChoosed(userlist[i].id, choosedPeople.data)) {
...
//显示附件
    ...
    //获取各学校校长列表
    documentModify.getSchoolHeadmasterList=function()
    {
        $.ajax({
            ...
            success:function(data)
            {
                for(var i=0;i<data.length;i++)
                {
                    ...
                    for (var j = 0; j < data.length; j++) {
                        if (checkIfHave(data[j].id, prePublishList)) {
                            if (!checkIfChoosed(data[j].id, baseChoosedPeople.data)) {
    ...
    documentModify.getSchoolHeadmasterList();
