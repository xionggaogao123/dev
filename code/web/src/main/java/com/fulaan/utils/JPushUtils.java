	   ...
	   public void pushNotice(final NoticeEntry e,final String teacherName,final String schoolId)
	   {
		   Thread t = new Thread(new Runnable() {
               ...
               public void run() {
            	   ...
            	   if(null!=info)
            	   {
//            		   if(info.getIsAllStudent()==Constant.ONE || info.getIsAllTeacherAndStudent()==Constant.ONE ) //学生
//            		   {
//            			   tags.add(schoolId);
//            			   tags.add(String.valueOf(UserRole.STUDENT.getRole()));
//            			   Audience audience =Audience.tag_and(tags);
...
//            		   }
//            		   if(info.getIsAllTeacher()==Constant.ONE || info.getIsAllTeacherAndStudent()==Constant.ONE) //老师
//            		   {
//            			   tags.clear();
//            			   tags.add(schoolId);
	   ...
	   public void pushHomeWork(final HomeWorkEntry e,final String teacher)
	   {
		   Thread t = new Thread(new Runnable() {
              ...
              public void run() {
            	   ...
            	   Map<ObjectId, ClassEntry> classMap =classService.getClassEntryMap(classIds,new BasicDBObject("stus",1) );
            	   
            	   
            	   for(Map.Entry<ObjectId, ClassEntry> entry:classMap.entrySet())
            	   {
            		   userIdSet.addAll(entry.getValue().getStudents());
            	   }
            	   
       		       List<InterestClassEntry>  intList= interestClassService.findInterestClassEntrysByIds(classIds,new BasicDBObject("tid",1).append("stl", 1));
 				   for(InterestClassEntry intc:intList)
 				   {
 					  userIdSet.addAll(MongoUtils.getFieldObjectIDs(intc.getInterestClassStudents(), "sid"));
 				   }
