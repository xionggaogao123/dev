...
function initFileData() {
    ...
    $.ajax({
        ...
        success: function (res) {
            var dto=res.dto;
            if(dto.picFile!=null&&dto.picFile!=''){
                createInfo.pics.push(dto.picFile);
                createInfo.picNames.push(dto.picName);
                var html='<div id="'+dto.picId+'">';
                html+='<img src="/img/notic/inform-complie.png">';
                html+='<div>';
                html+='<span>'+dto.picName+'</span><span class="inform-kb"></span>';
                html+='<span class="inform-YY-SC" onclick="deletePic(\''+dto.picId+'\',\''+dto.picFile+'\')">删除</span>';
                html+='</div>';
                html+='</div>';
            ...
            if(dto.phonePicFile!=null&&dto.phonePicFile!=''){
                createInfo.phonePics.push(dto.phonePicFile);
                createInfo.phonePicNames.push(dto.phonePicName);
                var html1='<div id="'+dto.phonePicId+'">';
                html1+='<img src="/img/notic/inform-complie.png">';
                html1+='<div>';
                html1+='<span>'+dto.phonePicName+'</span><span class="inform-kb"></span>';
                html1+='<span class="inform-YY-SC" onclick="deletePhonePic(\''+dto.phonePicId+'\',\''+dto.phonePicFile+'\')">删除</span>';
                html1+='</div>';
                html1+='</div>';

                $(".infrom-PTP").append(html1);
                $(".infrom-PTP").show();
            }

            if(dto.docFile!=null&&dto.docFile.length>0){
                var html="";
                for(var i=0;i<dto.docFile.length;i++){
                    createInfo.docs.push(dto.docFile[i].value);
                    createInfo.docsNames.push(dto.docFile[i].name);
                    html='<div id="'+dto.docFile[i].idStr+'">';
                    html+='<img src="/img/notic/inform-complie.png">';
                    html+='<div>';
                    html+='<span>'+dto.docFile[i].name+'</span><span class="inform-kb"></span>';
                    html+='<span class="inform-YY-SC" onclick="deleteDoc(\''+dto.docFile[i].idStr+'\',\''+dto.docFile[i].value+'\')">删除</span>';
                    html+='</div>';
                    html+='</div>';
                }
                $(".infrom-FJ").append(html);
