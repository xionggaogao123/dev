...
<div id="content" class="clearfix">
  ...
  <div class="col-right">
    ...
    <div>
      <select style="height: 50px;" id="userid">
        <option value="0">全部</option>
        <c:forEach items="${user}" var="userinfo">
          <option value="${userinfo.id}">${userinfo.userName} ${userinfo.schoolName}</option>
        </c:forEach>
      </select>
    </div>
    ...
    <script type="text/template" id="hyyq">
      ...
      <li class="clearfix">
        <div class="clearfix">
          ...
          <h3>{{=value.senderName}}|{{=value.roleName}}
            {{? value.unread ==1 }}
            <ii class="redpointt"></ii>
            {{?}}
            <span>{{=value.sendingTime}}</span></h3>
          <a href="/yunying/replyMessage.do?replyId={{=value.senderId}}&userid={{=value.userid}}"><p>{{=value.content}}</p></a>
        </div>
        <div class="yxj"><a href="/yunying/replyMessage.do?replyId={{=value.senderId}}&userid={{=value.userid}}">回复</a> | <span
