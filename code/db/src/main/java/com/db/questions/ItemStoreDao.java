...
public class ItemStoreDao extends BaseDao{
	 ...
	 * 全查询
	 */
	public List<ItemStoreEntry> fandAll(String questionTopic, int skip, int limit){
 		List<ItemStoreEntry> resultList = new ArrayList<ItemStoreEntry>();
		BasicDBObject query = null;
		DBObject orderBy =new BasicDBObject("pti",Constant.DESC);
		if("0".equals(questionTopic)){
			query = new BasicDBObject();
		}else{
			query = new BasicDBObject().append("qt",questionTopic );
		}

		List<DBObject> dbObjects = find(MongoFacroty.getCloudAppDB(),Constant.COLLECTION_ITEM__STORE,query,Constant.FIELDS,orderBy,skip,limit);
	...
	public int fandAllCount(String questionTopic){
		BasicDBObject query = new BasicDBObject();
		if(!"0".equals(questionTopic)){
			query.append("qt",questionTopic );
		}
		return count(MongoFacroty.getCloudAppDB(),Constant.COLLECTION_ITEM__STORE,query);
	}
	...
	public List<ItemStoreEntry> getItem(String id,String propertyType,String questionTopic,int isSaved){
		 List<ItemStoreEntry> resultList = new ArrayList<ItemStoreEntry>();
			BasicDBObject query = null;
			DBObject orderBy =new BasicDBObject("pti",Constant.DESC); 
			if(!StringUtils.isBlank(propertyType) && !"0".equals(questionTopic)){
